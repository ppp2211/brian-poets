	...
	for gi in root.findall(graphversion+'GraphInstance'):
		for edgeI in gi.findall(graphversion+'EdgeInstances'):	
			for edge in edgeI.findall(graphversion+'EdgeI'):	
				...
				regex = re.compile(r'(firefly_s[0-9]+_[0-9]+_[0-9]+):flash_in\-(firefly_s[0-9]+_[0-9]+_[0-9]+):flash_out')
				m = regex.search(pathstr)
				if m:
					linkflag=True
					o+= '\t\t{\"source\": \"'+ str(m.group(1)) +'_flash_log\", \"target\": \"'+str(m.group(2))+'_flash_log\", \"value\":0.001},\n'
	if linkflag:
